# app.R — auto-load sample_info_annotated.txt; card tags below image; card tags from dropdown vocab (incl. auto "Distant (any)")
# Collapsed mode, cohort filtering & gray-out behavior preserved.
    ...
    div(class = "app-wrap",
      div(class = "left-col",
        ...
        tags$p(class = "help-text",
               ...
               "Optionally include ", code("Patient_ID"), ", ", code("met_treated"), ", ", code("met_timing"),
               " for the tag cloud."
        ),
...
server <- function(input, output, session) {
  ...
  # Patient tag-cloud (left) — optional
  ...
  card_tag_vocab <- reactive({
    ...
    time_opts  <- c("Synchronous","Metachronous","Meta after sync","Meta-after-sync") # normalize later
  ...
  card_tag_colors <- reactive({
    voc <- card_tag_vocab()
    if (!length(voc)) return(setNames(character(), character()))
    hues <- seq(15, 375, length.out = length(voc) + 1L)[1:length(voc)]
  ...
  # NEW: Auto-load sample_info_annotated.txt if present
  ...
  # Helper: patient-level tags for left cloud (unchanged)
  ...
  # NEW: card tags derived from per-sample attributes present in that tree's matrix
  card_tags_for_tree <- function(label) {
    obj <- dist_list(); sa <- sample_attrs()
    if (is.null(obj) || is.null(sa$type) || !(label %in% names(obj))) return(character())
    dmat <- obj[[label]]
    rn <- rownames(if (inherits(dmat, "dist")) as.matrix(dmat) else as.matrix(dmat))
    if (is.null(rn) || !length(rn)) return(character())

    # collect present values from samples in this tree
    typ <- sa$type[rn]; trt <- sa$treated[rn]; tim <- sa$timing[rn]
    typ <- typ[!is.na(typ) & nzchar(typ)]
    trt <- trt[!is.na(trt) & nzchar(trt)]
    tim <- tim[!is.na(tim) & nzchar(tim)]

    # normalize timing label variants
    tim <- gsub("^Meta[- ]?after[- ]?sync$", "Meta after sync", tim, ignore.case = TRUE)

    type_tags  <- intersect(unique(typ), c("Peritoneum","Liver","Locoregional"))
    # Auto 'Distant (any)' if ANY type NOT in Normal/Primary/Locoregional is present
    if (any(!(unique(typ) %in% c("Normal","Primary","Locoregional","Peritoneum")))) {
      type_tags <- union(type_tags, "Distant (any)")
    }

    time_tags  <- intersect(unique(tim), c("Synchronous","Metachronous","Meta after sync"))
    treat_tags <- intersect(unique(trt), c("Untreated","Systemic chemo","Sys-chemo after untreated","HIPEC"))

    unique(c(type_tags, time_tags, treat_tags))
  }
  ...
  build_current_specs <- reactive({
    ...
    # selection for current cohort view
  ...
  output$gallery <- renderUI({
    ...
    if (length(sel)) {
      show <- Filter(function(x) {
        tgs <- tags_for_tree(x$label) # left-panel tags
        all(sel %in% tgs)
      }, show)
    }
    ...
    lapply(show, function(it) {
      lbl <- it$label
      # NEW: tags displayed under each card (from dropdown vocab/sample attrs)
